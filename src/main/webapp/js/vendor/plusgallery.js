...
(function ($){
	$.fn.plusGallery = function(options){
		...
		var pg = {
			...
			limit: 1000, //Limit of photos to load for gallery / more that 60 is dumb, separate them into different albums
			...
			getDataAttr: function(){
				...
				{
					pg.accessToken = $.cookie("facebook-token");	
				}	
        ...
        var touchClass;
				...
				lmnt.append(
					'<ul id="pgalbums" class="unstyled clearfix"></ul>' +
					'<div id="pgthumbview">' +
						'<ul id="pgthumbs" class="unstyled clearfix"></ul>' +
					'</div>'
				);
				$('body').prepend(
				...
				if(pg.albumTitle === true) {
					$('#pgthumbview').prepend('<ul id="pgthumbcrumbs" class="unstyled clearfix"><li id="pgthumbhome">&laquo;</li></ul>');
				}
			},
            ...
            imgHTML = '<img src="' + galleryImage + '" title="' + galleryTitle + '" title="' + galleryTitle + '" class="pgalbumimg">';
					...
					$('#pgalbums').append(
						'<li class="pgalbumthumb">' +
							'<a href="' + galleryJSON + '" class="pgalbumlink">' + imgHTML + '<p >' + galleryTitle + '</p></a>' +
